...
* зареєструватися на сайтеі [Нової Пошти](http://novaposhta.ua)
* На [сторінці налаштувань](https://my.novaposhta.ua/settings/index#apikeys) в особистому кабінеті сгенерувати ключ для роботы з API 

Після отримання ключа API надається можливість використовувати усі методи класа [офіційний сайт з документації](https://my.novaposhta.ua/data/API2-200215-1622-28.pdf)
...
# Використання 
## Підключення класа при установці через composer
```php
...
Клас за замовучуванням знаходиться в namespace `\LisDev\Delivery`. При створенні екземпляру класа необхідно
або використовувати Full Qualified Class Name:
```php
$np = new \LisDev\Delivery\NovaPoshtaApi2('Ваш_ключ_API_2.0');
```
або вказати використовуємий namespace у секції use:
```php
...
Більш детальну інформацію по роботе з namespace можно отримати [на сайті документації php](https://www.php.net/manual/ru/language.namespaces.rationale.php)

## Створення екземпляру класа (з розширеними параметрами)
Якщо потрібно отримувати дані на мові, відмінної від загарбницької, рекомендується виключати Exception при помилці запросу, або за відсутності установленої бібліотеки curl на сервері
```php
$np = new NovaPoshtaApi2(
	'Ваш_ключ_API_2.0',
	'ru', // Мова повертаємих даних: ru (default, sadly) | ua | en
...
// Інформація склада овідправника
...
$result = $np->newInternetDocument(
    ...
    array(
        ...
        // Замість FirstName, MiddleName, LastName можно ввести зареєстровані ПІБ відправника чи названі фірми для юридичних особ
...
# Тести
Актуальні тести та приклади використовуваня класа знаходятся у файлі `tests/NovaPoshtaApi2Test.php`
